...
void parse(cStringMap& patterns, int len, char *text) {
  ...
  for (int ii=0; ii<strlen(text) - len + 1; ii++) {
    ...
    strncpy(substring,text+ii,len); // copy a piece of <text> into <substring>
...
int main()
{
  ...
  setenv("QUERY_STRING","len=1&text=hi%20there%20how%20are%20you?",1);
  initializeGet(Get);
  length = atoi(Get["len"].c_str()); // the length inputted by the user
  strcpy(text,Get["text"].c_str());  // the text inputted by the user
